...
export const fetchIdoPools = async (poolId?: number) => {
  ...
  if (poolId) {
    ...
    const findLocal = poolInfoChain.find((item) => item.poolId === poolId) ?? IdoDefaultData
    const claimConfigItem = (poolClaimInfoList?.poolList ?? []).find((item) => item.poolId === poolId)
